...
| [015](015-dashboard-redesign.md) | Dashboard redesign — the wireframe's grammar, not its chat product | draft | A supplied wireframe is a generic AI-chat home screen ("New Chat", freeform prompt box, Chat/Library/Team nav, relative timestamps); this spec takes its visual weight — greeting hero, quick-action cards, an activity feed — and maps each element onto routes and mutations specs/001–004 already decided, rejecting the rest: no chat surface, `Explore templates` stays dropped (specs/001 §4, rule 1), relative timestamps stay rejected (lib/utils.ts, specs/003 §2.3), and a global search box stays unbuilt because specs/004 §3.1 already ruled the one real search box out of the URL. Restores the client-side greeting specs/003 §3.10 deferred until authentication existed. |
